...
## 2025 Protocol Pivot Roadmap (Updated Nov 2025)

| Sprint | Theme | Primary Deliverables | Status |
| --- | --- | --- | --- |
| 9 | **ARI + Runtime Registry** | P2P ARI executor default, runtime-registry API/metrics, actual-cost accounting | ✅ Complete |
| 10 | **Milestone Escrow Activation** | On-chain milestone releases, approval APIs, conditional/oracle rules, updated docs | 🚧 In progress |
| 11 | **Dispute Automation** | Pallet-dispute v1, arbitration APIs, evidence submission, dispute dashboards | ⏳ Planned |
| 12 | **Insurance + Risk Markets** | Pallet-insurance, premium oracle, pool accounting, risk analytics | ⏳ Planned |

> Older roadmap entries below remain as a historical archive. Each new sprint replaces the equivalent legacy section when the scope changes.
...
### SPRINT 15-16: Operator Console (Control Plane UI)
**Goal**: Give operators and protocol engineers a first-class control plane for the network.

**Week 1 (Sprint 15)**: Orchestrator & runtime views
- Runtime registry dashboard (from `/api/v1/runtime-registry`)
- Task queue overview with filtering
- Health panels for orchestrator, blockchain, and P2P
- Basic drill-down into single runtime / node

**Week 2 (Sprint 16)**: Economic and market views
- Escrow and payment status dashboard
- Auction activity view (VCG outcomes, bid distribution)
- Reputation and slashing panels
- Exportable metrics for incident review

**Deliverables**:
- ✅ Operators can see live runtimes and tasks
- ✅ Economic state is observable from a single pane of glass
- ✅ Ready to host on `console.ainur.network` (or equivalent)

---

### SPRINT 17-18: Developer & Research Dashboard
**Goal**: Provide developers and researchers with tools to inspect workflows, traces, and learning behavior.

**Week 1 (Sprint 17)**: Task and workflow introspection
- Task list with status, cost, and path (which runtimes handled each step)
- Timeline view of complex workflows (coalitions, negotiations, subtasks)
- Integration with tracing (ARI calls, P2P messages)

**Week 2 (Sprint 18)**: Learning & experimentation tools
- MARL/Q-routing introspection (Q-values, policies, win rates)
- Experiment management (A/B of strategies, rollout flags)
- Export of anonymized traces for offline research

**Deliverables**:
- ✅ Developers can debug complex multi-agent workflows
- ✅ Researchers can measure learning outcomes and adjust policies
- ✅ Dashboards aligned with the “Nexus” HMARL layer
...
### SPRINT 57-58: Nexus HMARL – Shared Context (Nexus-Context-v1)
**Goal**: Shared working memory for hierarchical multi-agent RL
...
### SPRINT 59-60: Nexus HMARL – Peer Learning (Nexus-Learn-v1)
**Goal**: Peer learning and decentralized policy improvement
...
### SPRINT 73-76: Operator & Analytics Consoles
**Goal**: Production-grade web consoles for operating the protocol.

**Week 1-2 (Sprint 73-74)**: Core operator console
- Multi-cluster orchestrator view
- Runtime registry explorer (filter by shard, capability, health)
- Escrow, auction, and payment overviews
- Live incident console with logs/metrics cross-linking

**Week 3-4 (Sprint 75-76)**: Analytics and exec dashboards
- High-level KPIs (agents online, tasks/day, failure rates)
- Economic KPIs (VCG efficiency, TVL, revenue, payouts)
- SLA and SLO dashboards for enterprises
- Hooks for future mobile/companion views if needed

**Deliverables**:
- ✅ Control-plane UI for operators and SREs
- ✅ Executive-level analytics dashboards
- ✅ Ready for on-call and NOC usage
...
- ✅ $10M+ TVL
- ✅ 99.99% uptime
- ✅ Global adoption across multiple industries and regions
